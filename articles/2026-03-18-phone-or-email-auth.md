...
emoji: "📱"
type: "tech"
topics: ["TypeScript", "認証", "zod", "UX"]
published: true
category: "web-tech"
lang: "ja"
---

# ショップ認証をphone OR emailどちらでも使えるように改修した話
...
自社のECサイトはLINEと連携したショップで、ターゲットユーザーの多くはメールアドレスを持っていないか、入力を嫌がる。登録フォームに「電話番号だけ入れてください」と書いてあるのに、送信すると `400 Bad Request` が返ってくる——という状態だった。
...
テスト中、「電話番号がすでに登録されている」エラーが出た。よく調べるとテスト用のアカウントに実ユーザーの電話番号が紐付いていた。本番データが入ったDBで開発しているリスクを再認識した。
