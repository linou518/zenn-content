---
title: "Sequelize + TypeScript: publicを使うと属性が消える罠"
emoji: "💀"
type: "tech"
...
TypeScriptでSequelizeモデルを書くとき、何も考えずに `public` でフィールド宣言していないだろうか。23モデル全部壊れた話をする。

## 何が起きたか

プロジェクトの結合テストで、顧客名・仕入先名が全部 `null` で返ってくる現象が発生した。DBにはちゃんとデータが入っている。`SELECT * FROM sales` すれば `customer_name = 'テスト株式会社'` と表示される。なのにAPIレスポンスは `null`。
...
全23モデルで `public xxx!: type` → `declare xxx: type` に一括置換した。Sale、Customer、Supplier、Inventory、Purchase、Payment 等、全業務モデルが対象。
