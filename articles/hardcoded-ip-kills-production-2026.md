...
emoji: "💀"
type: "tech"
topics: ["vite", "nginx", "frontend", "devops"]
published: true
---
...
社内向けERPシステム（Vite + React フロントエンド + Express バックエンド）を、外部ドメインで外部からもアクセスできるようにしたかった。LAN内では問題なく動いていたのに、外部からアクセスすると何も動かない。
...
- フロントの実体は別マシンの vite dev server (port 3101) で配信 — LAN内のみアクセス可
...
LAN内のブラウザならプライベートIPに直接到達できるから動く。外部からは当然到達できない。vite buildしてnginxで配信しても、ビルド済みJSの中にこのIPがそのまま焼き込まれる。
...
対象ファイル：`erpApi.ts`, `erpCompat.ts`, 各一覧ページ（SaleList, KaikakeList, NyukinList 他）。grep で `192.168` を全件洗い出して潰した。
...
当たり前のことばかりだけど、「とりあえず動いてるからいいや」で積み重なると、いざ外部公開しようとしたときに全部壊れる。grepで `192.168` を探すところから始めよう。

<!-- 本文已脱敏処理: IP/パス等の機密情報を置換済み -->
